...
def results_checker(file_name, skip_duplicates = False, naive_score = False, confusion_matrix = False):
    ...
    tp_total = sum(v["TP"] for v in per_class_results.values())
    fp_total = sum(v["FP"] for v in per_class_results.values())
    fn_total = sum(v["FN"] for v in per_class_results.values())
    micro_f1 = (2 * tp_total) / max(1, (2 * tp_total + fp_total + fn_total))

    print(f"Total entries: {n} \n Accuracy: {round(correct/n, 4)} \n "\
          + f"Weighted F1: {round(weighted_f1, 4)} \n Unweighted F1: "\
          + f"{round(unweighted_f1, 4)} \n Micro F1: {round(micro_f1, 4)} "\
