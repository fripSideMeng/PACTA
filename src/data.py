...
cll_v2 = ['SportsEvent', 'Locality', 'Museum', 'Film', 'Person', 
          'mass', 'Album', 'Language', 'duration', 'brand', 'Website', 
          'Company', 'Book', 'Image', 'MusicalArtist', 'Organisation', 
          'Song', 'TelevisionEpisode', 'distance', 'PostalCode', 'Event', 
          'WrittenWork', 'rating', 'Restaurant', 'price', 'List', 
          'gender', 'Category', 'dateTime', 'address', 'Hotel', 'Region', 
          'Currency', 'date', 'boolean', 'energy', 'SportsTeam', 'Street', 
          'TelevisionShow', 'time', 'weight', 'Place', 'Identifier', 'day', 
          'Country', 'review']
...
abbrev_map = {**{s[1:] : s for s in cll}, **{s[1:] : s for s in clt}, 
              **{s[2:] : s for s in cll if len(s) > 5}, 
              **{s[2:] : s for s in clt if len(s) > 5}}

abbrev_map_v2 = {**{s[1:] : s for s in cll_v2}, 
                 **{s[2:] : s for s in cll_v2 if len(s) > 5}}

context_labels = {"name" : "context_labels", "label_set" : cll, 
                  "dict_map" : label_dict_map_full, 'abbrev_map' : abbrev_map}

context_labels_v2 = {"name" : "context_labels_v2", "label_set" : cll_v2, 
                     "dict_map" : label_dict_map_full_v2, 
                     'abbrev_map' : abbrev_map_v2}

context_labels_trim = {"name" : "context_labels_trim", "label_set" : clt, 
                       "dict_map" : label_dict_map, 'abbrev_map' : abbrev_map}

numeric_labels = ['currency', 'price', 'Number', 'Integer', 'IdentifierAT', 
                  'QuantitativeValue', 'Duration', 'priceRange', 'postalCode', 
                  'MonetaryAmount', 'Mass', 'CategoryCode', 'weight', 'unitCode', 
                  'Energy', 'Distance', 'workHours', 'typicalAgeRange', 
                  'telephone', 'faxNumber', 'zipCode', 'DateTime', 'Date', 
                  'Time', 'PostalAddress', 'openingHours']

numeric_labels_v2 = ['currency', 'Time', 'weight', 'Mass', 'Date', 
                     'Distance', 'Duration', 'DateTime', 'postalCode', 
                     'price', 'Energy']
always_numeric_labels_v2 = []
...
context_labels_small = {"name" : "context_labels_small", "label_set" : cls, 
                        "dict_map" : label_dict_map_small, 'abbrev_map' : abbrev_map}

sotab_integer_labels = ['DateTime', 'Date', 'Integer', 'telephone', 
                        'faxNumber', 'Energy']
...
sotab_other_labels = ['Identifier', 'email', 'URL', 'WebHTMLAction', 
                      'Photograph', 'category', 'text']

sotab_top_hier = {"integer" : sotab_integer_labels, "float" : sotab_float_labels, 
...
def prompt_context_insert(context_labels: str, context : list[str], max_len : int = 2000, 
                          model : str = "gpt-3.5", args : dict = dict(), 
                          do_kshot : bool = False, rows: list[str] = None):
  if "gpt" in model:
  ...
  if "chorusprompt" in model:
    ...
    s = f'Answer the question based on the task below. If the question cannot be answered '\
        + f'using the information provided, answer with "I don\'t know". \n Task: Classify '\
        + f'the column{table_src_str} given to you into only one of these types: '\
        + f'{context_labels} \n {addl_instr} \n Input column: {context} \n{ocs} Type: \n'
    ...
    if "llama" in model:
      ...
      s = f'Instruction: From the following list of labels, choose one that best '\
          + f'describes the column.\nLabels: {context_labels}\nColumn: {context}\n'\
          + f'Answer: \n'
    ...
    if rows is None:
      entries = "[" + ", ".join(context).replace("[", "")\
                .replace("]", "").replace("'", "") + "]"
    else:
      if args["of"]:
        row_context_1, row_context_2 = '\n'.join(rows[0][1:]), '\n'.join(rows[1][1:])
      else:
        entries = "[" + ", ".join(rows).replace("[", "")\
                  .replace("]", "").replace("'", "") + "]"
    ...
    if model == "opt-iml-max-30b-zs":
        ...
        if "llama3" not in model:
          if rows is None or ("of" in args and not args["of"]):
            if args["template"] == "origin":
              s = 'INSTRUCTION: From the following list of labels, choose one '\
                  + f'that best describes the column.\nLabels: {lb}.\nColumn: '\
                  + f'{entries}\nAnswer: \n'
            elif args['template'] == 'origin1':
              s = f'Column: {entries}\nWhich of the following labels matches best?\n{lb}'
            elif args['template'] == 'origin2':
              s = 'Select the option which best describes the input.\n'\
                  + f'INPUT: {entries}.\nOPTIONS:\n{lb}\n'
            elif args["template"] == "alter1":
              s = 'Pick the column\'s class. I mean if you want to. It would be cool, '\
                  + f'I think. Anyway, give it a try, I guess?\nHere\'s the column itself! '\
                  + f'{entries}\nAnd, um, here are some column names you could pick from '\
                  + f'... {lb}\nOk, go ahead!\n'
            elif args['template'] == 'alter2':
              s = f"Given the input column: {entries}, choose the most appropriate "\
                  + f"label from the following options and return only the label.\nOPTIONS: {lb}"
            elif args['template'] == 'alter3':
              s = f"Labels: {lb}\nColumn: {entries}\nSelect the label that best describes the column."
            else:
              s = f"Determine the correct label for this column.\nLabels: {lb}\nColumn: {entries}"
          else:
            s = f"INSTRUCTION: Given the following two entries from a table column:\nEntry "\
                + f"one:\n{rows[0][0]}\nOther columns values from the same row:\n"\
                + f"{row_context_1}\nEntry two:\n{rows[1][0]}\nOther columns values from "\
                + f"the same row:\n{row_context_2}\nInfer one label from the following list "\
                + f"that best describes the entries based on other columns values.\n"\
                + f"Labels: {lb}\nAnswer: \n"
        else:
          s = f'<|begin_of_text|><|start_header_id|>system<|end_header_id|>\n\n'\
              + f'You are a concise classifier. Respond with a single label only without explanation.\n\n'\
              + f'<|eot_id|><|start_header_id|>user<|end_header_id|>\n\n'\
              + f'Column entries: {entries}\n\nPlease label this column with one of '\
    ...
    if args["template"] == "origin":
      ...
      s = f'Examine the data: {entries}. Then, from the options '\
          + f'provided below, pick one of them that matches the data.\n'\
          + f'Options: {lb}'
  if "internlm" in model or "speechless" in model:
    s = s.replace("[", "").replace("]", "").replace("'", "")
  if args.get('tokenizer', None) is not None:
    inputs = args["tokenizer"].encode(s, return_tensors="pt", add_special_tokens=True)
    target_len = len(inputs[0])
    if rows is None:
      while target_len > max_len and len(context) > 1:
        context.pop()
      ...
      while target_len > max_len and len(rows) > 1:
        rows.pop()
        entries = "[" + ", ".join(rows).replace("[", "").replace("]", "").replace("'", "") + "]"
        if args["template"] == "origin":
          s = f'INSTRUCTION: From the following list of labels, choose one '\
              + f'that best describes the column.\nLabels: {lb}.\nColumn: '\
              + f'{entries}\nAnswer: \n'
        elif args['template'] == 'origin1':
          s = f'Column: {entries}\nWhich of the following labels matches best?\n{lb}'
          ...
          s = f'Pick the column\'s class. I mean if you want to. It would be cool, '\
              + f'I think. Anyway, give it a try, I guess?\nHere\'s the column itself! '\
              + f'{entries}\nAnd, um, here are some column names you could pick from '\
              + f'... {lb}\nOk, go ahead!\n'
        elif args['template'] == 'alter2':
          ...
          s = f"Labels: {lb}\nColumn: {entries}\nPick the most proper label given the input column."
        else:
          s = f"Choose the most proper label from the following options and return "\
              + f"only the label, given the input column. OPTIONS: {lb}\nColumn: {entries}"
        inputs = args["tokenizer"].encode(s, return_tensors="pt", add_special_tokens=True)
        target_len = len(inputs[0])
  return s
...
def get_df_sample(df, rand_seed, val_indices, len_context, min_variance=1, 
                  replace=False, full=False, other_col=False, max_len=8000, 
                  method=[], coherence_scores=None, args=dict()):
    column_samples = {}
    ...
    for idx, col in enumerate(df.columns):
      ...
      if "simple_random_sampling" in method:
        ...
        sample_list = colvals.sample(n=ss_orig, replace=True, random_state=rand_seed, 
                                     weights=coherence_scores[idx]).tolist()
      else:
        # archetype sampling
        sample_list = []
        ...
        if len(sample_list) > ss_orig:
          ...
          indices = np.random.choice(np.arange(len(sample_list)), size=ss_orig, 
...
amstr_renamed_class = ['Newspaper or Publication', 'Numeric Identifier', 'Town', 'State', 
                       'Headline','Author Byline'] + \
...
amstr_zs_context_labels = {"name" : "amstr_zs", "label_set" : amstr_renamed_class, 
                           "dict_map" : {c : c for c in amstr_renamed_class}, 
                           "amstr_map" : amstr_classname_map}
...
pubchem_classes = ['author_given_name', 'Molecular_Formula', 'book_title', 'cell_altlabel', 
                   'book_subtitle', 'disease_altlabel', 'book_creator', 'author_family_name', 
                   'IUPAC_name', 'taxonomy_label', 'InChI', 'SMILES', 'patent_abstract', 
                   'organization_fn', 'book_isbn', 'concept_broader', 'journal_title', 
                   'concept_preflabel', 'author_fn', 'journal_issn', 'patent_title']
...
t2d_label_list = ["swimmer", "cricketer", "mountain", "Country", "Bird", 
                  "Company", "Monarch", "Mountain", "RadioStation", "Saint", 
                  "Currency", "AdministrativeRegion", "VideoGame", "Airline", 
                  "City", "Building", "Airport", "Animal", "Film", "Wrestler", 
                  "Hospital", "PoliticalParty", "Lake", "Person", "Plant", 
                  "Museum", "BaseballPlayer", "University", "Novel", 
                  "Scientist", "Newspaper", "Hotel", "AcademicJournal", 
                  "Election", "Book", "Work", "FictionalCharacter", "GolfPlayer", 
                  "TelevisionShow", "Mammal", "governmenttype", "moviedirector", 
                  "language", "genre"]

t2d_label_renamed = ["swimmer", "cricketer", "mountain", "nation", "Bird", 
                     "Company", "Roman Emperor", "Mountain", "RadioStation", 
                     "Saint", "Currency", "U.S.State", "VideoGame", "Airline", 
                     "City", "Building", "Airport", "Animal", "Film", "Wrestler", 
                     "Hospital", "PoliticalParty", "Lake", "Person", "Plant", 
                     "Museum", "BaseballPlayer", "University", "Novel", 
                     "Scientist", "Newspaper", "Hotel", "AcademicJournal", 
                     "Election", "Book", "Work", "FictionalCharacter", 
                     "GolfPlayer", "TelevisionShow", "Mammal", "SystemOfGovernment", 
                     "moviedirector", "language", "music or video game genre"]
...
viznet_chorus_labels = [
    "artist", "address", "city", "director", "album", "currency", 
    "product", "isbn", "manufacturer", "birthDate",
]
...
viznet_classes = ['address', 'affiliate', 'affiliation', 'age', 'album', 'area', 'artist', 
                  ...
                  'credit', 'currency', 'day', 'depth', 'description', 'director', 'duration', 
                  'education', 'elevation', 'family', 'file Size', 'format', 'gender', 
                  'genre', 'grades', 'industry', 'isbn', 'jockey', 'language', 'location', 
                  'manufacturer', 'name', 'nationality', 'notes', 'operator', 'order', 
                  'organisation', 'origin', 'owner', 'person', 'plays', 'position', 'product', 
                  'publisher', 'range', 'rank', 'ranking', 'region', 'religion', 'requirement', 
                  'result', 'sales', 'service', 'sex', 'species', 'state', 'status', 'symbol', 
                  'team', 'team Name', 'type', 'weight', 'year']
...
viznet_zs_context_labels = {"name" : "viznet_zs", "label_set" : viznet_renamed_class, 
                            "dict_map" : {c : c for c in viznet_renamed_class}, 
                            "viznet_map" : viznet_classname_map}
...
def get_sample_prefix(s):
  if s == "SOTAB-91":
    ...
    return "sotab82_test"
  print("Sample path prefix not found")
  return None